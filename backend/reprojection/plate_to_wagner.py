...
def generate_graticule_path_wagner(
    spacing: float,
    transformer,
...
def reproject_svg(input_svg, output_svg, input_bounds, output_width=1800, padding=0.0, graticule_spacing=None, scale_bar_km=None):
    """
    Reproject SVG from Plate Carrée (equirectangular) to Wagner VII projection.
    ...
    # Create transformers
    forward_transformer = Transformer.from_crs("EPSG:4326", WAGNER_VII_CRS, always_xy=True)
            ...
            if cmd_upper == 'Z':
                ...
                while i + 6 < len(params):
                    ...
                    if crosses_antimeridian(prev_lon, curr_lon):
                        ...
                        new_ry = scale_y
                        new_path.append(
                            f"A {new_rx:.2f},{new_ry:.2f} {rotation:.2f} {large_arc} {sweep} {nx:.2f},{ny:.2f}"
                        )
    ...
    for element in root.iter():
        ...
        if element.tag.endswith('path') and 'd' in element.attrib:
            element_transform = transforms_map.get(element, np.eye(3))
            # Preserve existing styles - don't remove fill, stroke, or style attributes
            # Only transform the path data
    ...
    if graticule_spacing is not None and graticule_spacing > 0:
        graticule_d = generate_graticule_path_wagner(
    ...
    if scale_bar_km is not None and scale_bar_km > 0:
        ...
        scale_result = generate_scale_bar_wagner(
